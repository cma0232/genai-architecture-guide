...
<Callout type="warn" label="Important distinction">A prompt does not replace software architecture. The application still controls authentication, data access, tool execution, validation, persistence, and failure handling. Natural language guides model behavior inside boundaries created by software.</Callout>
...
<Flow>
  <FlowBox>Intent and constraints</FlowBox>
  <FlowArrow />
  <FlowBox>Context and architecture</FlowBox>
  <FlowArrow />
  <FlowBox>Model-assisted execution</FlowBox>
  <FlowArrow />
  <FlowBox>Tests, evals, and review</FlowBox>
</Flow>
...
<Quiz
  question="A team adds an LLM to classify support tickets. Which new architectural requirement is most specific to the GenAI component?"
  options={[
    { label: "A", text: "Every request must use HTTP" },
    { label: "B", text: "The database must be relational" },
    { label: "C", text: "The team must evaluate acceptable behavior across varied inputs and validate the generated result", correct: true },
    { label: "D", text: "The application no longer needs ordinary tests" },
  ]}
  explain="The correct answer is C. Model behavior varies with instructions and context, so the system needs evaluation across representative cases and validation before downstream use."
/>
