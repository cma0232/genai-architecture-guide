...
> **Agentic system behavior = Model capabilities + Harness design**
...
> **Design habit:** Give the agent the minimum environment required for the task. Separate read, test, write, deploy, and approve capabilities rather than bundling them into one powerful tool.
...
<details>
<summary><strong>Check your understanding:</strong> An agent repeatedly calls the same API because it does not know the request already succeeded. Which harness area should the team investigate first?</summary>

- **A.** The UI color palette
- **B.** State management and orchestration
- **C.** The model's training cutoff only
- **D.** The number of users in the database

**Answer: B.** The correct answer is B. The system is failing to record an observation and use it in the next control decision.
