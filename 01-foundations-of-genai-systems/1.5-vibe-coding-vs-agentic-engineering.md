...
<Callout type="tip" label="Litmus test">If the only evidence is "I clicked around and it looked right," the work is still at prototype verification—even if the prompt and model are sophisticated.</Callout>
...
<Analogy>A prototype is a sketch used to decide what building to construct. Agentic engineering is the structural plan, inspection process, and controlled construction site required to let people safely live inside it.</Analogy>
...
<Quiz
  question="A team uses an AI coding agent, requires a written spec, runs unit and integration tests, checks the agent's file changes, and blocks deployment when eval thresholds fall. Which mode best describes the process?"
  options={[
    { label: "A", text: "Vibe coding, because AI generated code" },
    { label: "B", text: "Manual development, because tests are present" },
    { label: "C", text: "Agentic engineering", correct: true },
    { label: "D", text: "Multi-agent architecture" },
  ]}
  explain="The correct answer is C. The process turns intent into constrained execution and repeatable evidence rather than relying on an eyeball test."
/>
